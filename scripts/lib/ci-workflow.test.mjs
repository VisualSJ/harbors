...
test('Kit finish workflow resolves the Task and checks the declared Kit boundary', async () => {
  const packageJson = JSON.parse(await readFile(packageUrl, 'utf8'));
  assert.equal(typeof packageJson.scripts['kit:boundary'], 'string');
  const finish = await readFile(new URL('.agents/skills/kit-workflow/scripts/finish-kit-change.sh', rootUrl), 'utf8');
  assert.match(finish, /task-status.mjs resolve/u);
  assert.match(finish, /--ready-for-pr/u);
  assert.match(finish, /kit:boundary/u);
  assert.doesNotMatch(finish, /plan-kit-releases|kit-publish/u);
});
