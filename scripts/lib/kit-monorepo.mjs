...
// Kits are loaded strictly from the repository kits/ directory. The slug is
// validated against the canonical pattern and the resolved physical directory
// must remain inside kits/ to prevent path traversal.
export async function loadTrustedMarketKit({ repositoryRoot, slug }) {
  ...
  if (packageJson.name !== manifest.id) {
    throw new Error(
      `Kit identity mismatch for ${slug}: package.json.name ${packageJson.name} does not match kit.json.id ${manifest.id}`,
    );
  }
  if (packageJson.version !== manifest.version) {
    throw new Error(
      `Kit version mismatch for ${slug}: package.json.version ${packageJson.version} does not match kit.json.version ${manifest.version}`,
    );
  }
  ...
  if (lockedPackage?.name !== manifest.id || lockedPackage?.version !== manifest.version) {
    throw new Error(
      `package-lock identity mismatch for ${slug}: lock name ${lockedPackage?.name}@${lockedPackage?.version} does not match descriptor ${manifest.id}@${manifest.version}`,
    );
  }
