...
Kit 源码由本地 `kits/` 目录发现。开发者先以 `npm run kit -- validate` 校验 descriptor 与声明文件，再以 `npm run kit -- pack` 生成 `.hkit`，并用 `npm run kit -- inspect` 检查制品内容。制品由本地路径或已导入的文件提供；合并源码不会触发额外的制品分发流程。
