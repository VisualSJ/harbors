...
Kit 只在本地开发与打包，PR 合并仅更新源码，不会自动创建制品或变更版本。版本字段仍须由
`kit.json` 和 `package.json` 保持一致，作为 descriptor 与制品校验的静态元数据。完整生命周期是：
...
的 PR。路径级 CI 至少检查被修改的 Kit；`kit-core`、Kit CLI 或其他共享构建面变化会触发所有官方 Kit CI。

开发 PR 应同步更新目标目录的 `kit.json`、`package.json` 和 `package-lock.json`。合并到 `main` 后仅更新源码；如需制品，在本地运行 validate、pack 和 inspect。
