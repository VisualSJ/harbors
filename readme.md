...
ITHARBORS 是一个插件优先的 Web 应用框架。Gateway 提供统一入口，Server 持有 Kit、会话与插件运行时，Client 在浏览器中渲染工作台。仓库只内置 `default` Kit；其他 Kit 从 `kits/` 目录自动发现，仅用于本地开发与打包。
...
- `scripts`：构建、检查、Task 与 Kit 本地打包工具。

Harbors 只支持 Web host，不包含 Electron、桌面打包、托盘、原生窗口、桌面更新或桌面 IPC。

## Kit 与插件

Kit 由 `kit.json` 和 `package.json` descriptor 描述；`distribution` 决定本地装配投影。插件通过 manifest 声明 Panel、Message、Menu 和 public assets，通过 `editor.plugin.define()` 注册生命周期。

仓库中的 Kit 从 `kits/` 目录自动发现，只在本地开发与打包。Kit 变更通过 `kit-change/<name>/<type>/<slug>` 分支提交 PR，合并后仅更新源码，不自动创建制品或变更版本。版本字段仍用于 descriptor 和制品校验；可用 `npm run kit -- validate`、`npm run kit -- pack` 与 `npm run kit -- inspect` 完成本地制品循环。
